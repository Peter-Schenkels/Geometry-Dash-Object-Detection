...
if __name__ == "__main__":
    ...
    templates.append(addTemplate('YOLOstuff/RegularBlock01.png', "platform_01", platform_threshold , (0, 255, 0)))
    templates.append(addTemplate('YOLOstuff/GridBlock01.png', "platform_02", platform_threshold , (0, 255, 0)))
    ...
    for template in templates:
        ...
        for roi in roi_list:
            if(roi.img.shape[0] > template[0].shape[0] and roi.img.shape[1] > template[0].shape[1]):
                matches = (cv.matchTemplate(cv.Canny(roi.img, 180, 200),template[0],cv.TM_CCOEFF_NORMED), template[1], w, h, template[2], template[3], roi.pos1)
